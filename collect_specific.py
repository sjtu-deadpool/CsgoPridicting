...
class CSGOMarketScraper:  
    ...
    def clean_date(self, date_str):  
        ...
        # 如果时间部分缺少分钟和秒，补全为 ":00:00"  
        if re.search(r'\d{2}:$', date_str):  # 检测是否以 "HH:" 结尾  
            date_str += "00:00"  
        ...
        for idx, item_name in enumerate(items_to_collect):  
            ...
            if price_history:  
                ...
                for entry in price_history:  
                    cleaned_date = self.clean_date(entry[0])  # 清理和补全日期字符串  
                    cleaned_data.append([cleaned_date, entry[1], entry[2]])  
